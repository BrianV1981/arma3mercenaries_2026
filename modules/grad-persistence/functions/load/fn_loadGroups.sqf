...

{
...

    {
        ...
        [{!isNull (_this select 0)}, {
            ...
            // --- A3M: DEACTIVATE AI ON LOAD (Using User's ACE Logic) ---
            [{
                params ["_unit"];
                if (alive _unit) then {
                    [_unit, true] call ACE_captives_fnc_setHandcuffed; // Force ACE handcuffs to beat VCOM
                    _unit allowDamage false; 
                };
            }, [_thisUnit], 5] call CBA_fnc_waitAndExecute;
