...
if (isNull _dialog) exitWith {
    diag_log "Error: grad_fortifications_DIALOG not found.";
};
...
private _type = if (!isNull _vehicleListCtrl && (_builder getVariable ["grad_fortifications_ui_lastSelectedContainer", _builder] != _builder)) then {
...
if (_type == "") exitWith {
    diag_log "Error: No fortification type selected.";
};
