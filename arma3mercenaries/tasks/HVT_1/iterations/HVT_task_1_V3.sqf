...
if (!_success) then {
    ...
    if (!isNil "_fallbackResult" && {_fallbackResult}) then {
         ...
         // Optionally set _success = true if needed later
    } else {
...
// --- *** REMOVED setWaypointStatements for cleanup *** ---
// Cleanup will be handled later using waitUntil
...
_HVT addEventHandler ["Killed", {
    ...
    private _taskCompletingSide = side _killer;
    {
        if (side _x == _taskCompletingSide) then {
            ...
            // --- Send Dynamic Text via remoteExec (mirroring sector script) ---
...
// --- *** HELICOPTER CLEANUP using waitUntil (Similar structure to Blackfish script) *** ---
// This block waits in the main script thread until the heli finishes its job
if (isServer) then { // Ensure this runs only on the server
    // Wait until the group exists and has more than 1 waypoint (meaning _wp2 is set)
    waitUntil { sleep 1; !isNull _heliGroup && { count waypoints _heliGroup > 1 } };

    if (!isNull _heliGroup) then {
        private _wp2Index = 1; // Index of the second waypoint (_wp2)

        diag_log format ["HVT Heli Cleanup Wait: Waiting for heli group %1 to complete waypoint %2.", groupID _heliGroup, _wp2Index];

        // Wait until the second waypoint is completed or the group/heli ceases to exist
        waitUntil { sleep 5; isNull _heliGroup || isNull _heli || { currentWaypoint _heliGroup > _wp2Index } };

        diag_log format ["HVT Heli Cleanup Wait: Condition met for group %1. Heli: %2", groupID _heliGroup, _heli];

        // Perform cleanup if objects still exist
        if (!isNull _heli) then {
            diag_log format ["HVT Heli Cleanup Wait: Deleting heli %1.", _heli];
            deleteVehicle _heli;
        };
        if (!isNull _heliGroup) then {
            diag_log format ["HVT Heli Cleanup Wait: Deleting group %1.", groupID _heliGroup];
            deleteGroup _heliGroup; // This also deletes the pilot
        };
    } else {
        diag_log "HVT Heli Cleanup Wait: Heli group was null before waiting could begin.";
    };
};
