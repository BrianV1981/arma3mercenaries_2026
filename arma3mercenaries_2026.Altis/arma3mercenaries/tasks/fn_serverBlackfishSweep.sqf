/*
    ...
    Executes on the dedicated server to securely find the HVT, spawn the Blackfish, and hand off remote control.
*/
...
// Create Blackfish High Above Target
private _spawnPos = [_exactPos select 0, _exactPos select 1, 800];
private _vehArray = [_spawnPos, random 360, "B_T_VTOL_01_armed_F", civilian] call BIS_fnc_spawnVehicle;
private _gunship = _vehArray select 0;
private _crew = _vehArray select 1;
private _grp = _vehArray select 2;

createVehicleCrew _gunship; // Ensure Arma 3 UAV AI is properly linked to the vehicle terminal

// Configure AI to prioritize flying over engaging
_grp setBehaviour "CARELESS";
_grp setCombatMode "BLUE";
_gunship flyInHeight 800;
...
[_gunship, _crew, _grp] spawn {
    ...
    if (alive _gunship) then {
        // Tell the gunship to fly away
        _grp setBehaviour "CARELESS";
        private _rtbPos = (getPos _gunship) getPos [5000, random 360];
    ...
    if (!isNull _gunship) then { deleteVehicle _gunship; };
