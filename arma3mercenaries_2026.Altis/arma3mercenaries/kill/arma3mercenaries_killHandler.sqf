...
A3M_fnc_serverHandleReward = {
    ...
    try {
        ...
        // A3M Phase 3: Mercenary Kill Tracker & Economy
        private _mercID = _instigator getVariable ["arma3mercenaries_aiUnit", ""];
        if (_mercID != "" && _sideKiller != _sideKilled && _sideKilled != 3) then {
            private _opforKillReward = if (isNil "arma3mercenaries_opforKillReward") then { 10000 } else { arma3mercenaries_opforKillReward };
            private _natoKillReward = if (isNil "arma3mercenaries_natoKillReward") then { 10000 } else { arma3mercenaries_natoKillReward };
            private _independentKillReward = if (isNil "arma3mercenaries_independentKillReward") then { 10000 } else { arma3mercenaries_independentKillReward };
            private _reward = 0;
            switch (_sideKilled) do {
                case 0: { _reward = round random _opforKillReward; };
                case 1: { if (_sideKiller == 0) then { _reward = round random _natoKillReward; }; };
                case 2: { if (_sideKiller == 0) then { _reward = round random _independentKillReward; }; };
            };
            if (_reward > 0) then {
                private _profile = [format["A3M_MERC_%1", _mercID], createHashMap, true] call A3M_fnc_dbGetSecure;
                if (count _profile > 0) then {
                    _profile set ["Kills", (_profile getOrDefault ["Kills", 0]) + 1];
                    _profile set ["CashCarried", (_profile getOrDefault ["CashCarried", 0]) + _reward];
