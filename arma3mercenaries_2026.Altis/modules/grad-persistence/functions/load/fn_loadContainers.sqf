...
// Process each saved container from the saved data
{
    ...
    [{!isNull (_this select 0)}, {
        ...
        _thisContainerData params [
            ...
            "_fortOwner", "_isGradMoneymenuStorage", "_mmOwner", "_thisLbmMoney", "_vars", ["_vehVarName", ""], ["_aceParentVar", ""]
        ];
        ...
        if (_aceParentVar != "") then {
            ...
            [{
                params ["_thisContainer", "_aceParentVar"];
                ...
                if (!isNil "_parentVeh" && {!isNull _parentVeh}) then {
                    ...
                    [_thisContainer, _parentVeh, true] call ace_cargo_fnc_loadItem;
                    if (_lockState >= 2) then { _parentVeh lock _lockState; };
                };
            }, [_thisContainer, _aceParentVar], 2] call CBA_fnc_waitAndExecute;
        };
