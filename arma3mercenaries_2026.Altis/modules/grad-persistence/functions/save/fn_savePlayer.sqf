...
// --- A.I.M. v812+ Architecture: SQLite Per-Entity Saving ---
private _unitDataHash = createHashMap;
...
if (_savePlayerInventory) then {
    ...
    _unitDataHash set ["inventory", _loadout];
};
...
if (_savePlayerDamage) then {
    if (isClass(configfile >> "CfgPatches" >> "ace_medical")) then {
        ...
        _unitDataHash set ["damage",_damage];
...
if (_savePlayerPosition) then {
    _unitDataHash set ["posASL",getPosASL _unit];
    _unitDataHash set ["dir",getDir _unit];
};

if (_savePlayerMoney) then {
    _unitDataHash set ["money",_unit getVariable ["grad_lbm_myFunds",0]];
    _unitDataHash set ["bankMoney",_unit getVariable ["grad_moneymenu_myBankBalance",0]];
};

private _thisUnitVars = [_allPlayerVariableClasses,_unit] call FUNC(saveObjectVars);
_unitDataHash set ["vars",_thisUnitVars];

// --- Push directly to SQLite via UID key ---
private _uniquePlayerKey = format ["%1_player_%2", _missionTag, _uid];
[_uniquePlayerKey, _unitDataHash] call A3M_fnc_dbSetSecure;

// We have achieved 100% SQLite purity. Profile namespace is no longer used for player state.