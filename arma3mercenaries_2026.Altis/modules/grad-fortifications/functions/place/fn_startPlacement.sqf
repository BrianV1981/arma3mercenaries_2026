// --- File: modules\grad-fortifications\functions\place\fn_startPlacement.sqf ---
// (Updated with state flag logic)
...
if !(isNull _container) then { _fort disableCollisionWith _container; }; // Added null check for safety

// Call onPlaceStart script if defined in config
if (_onPlaceStart != "") then {
...
// --- Set Initial Player State Variables ---
...
// *** ADDED: Initialize placement readiness to false ***
_builder setVariable ["grad_fortifications_placementReady", false, true];
...
// Call collision check PFH (Ensure fn_checkCollisionPFH exists and stores its handle correctly, e.g., in grad_fortifications_checkCollisionPFH_handle)
...
// Add the update Per-Frame Handler (stores handle in grad_fortifications_updatePFH_handle)
[_builder, _fort, _surfaceNormal] call grad_fortifications_fnc_addUpdatePFH;

// Add Mouse Event Handlers (will check the placementReady flag)
[] call grad_fortifications_fnc_addMouseEHs;

// Add Key Event Handlers
[] call grad_fortifications_fnc_addKeyEHs;

// *** ADDED: Set the placement ready flag after a short delay ***
[{
    ACE_player setVariable ["grad_fortifications_placementReady", true, true];
}, [], 0.1] call CBA_fnc_waitAndExecute; // Delay of 0.1 seconds
