/*  Processes buy request sent by client
*
*   remote executed by client
*/

params ["_baseConfigName", "_categoryConfigName", "_itemConfigName", "_buyer", "_price", "_vehiclespawn","_account"];

//detract from account
private _notEnoughMoney = if (typeName _account == "SIDE") then {
    if ([_account] call grad_lbm_fnc_getFunds < _price) then {
        "Teamaccount did not have enough money. Purchase canceled." remoteExec ["systemChat",_buyer,false];
        true
    } else {
        [_account,-_price] call grad_lbm_fnc_addFunds;
        false
    };
} else {
    false
};

if (_notEnoughMoney) exitWith {};

//check if in stock
_stock = [_baseConfigName, _categoryConfigName, _itemConfigName] call grad_lbm_fnc_getStock;
if (_stock <= 0) exitWith {
    [_buyer, _price, "Item out of stock. You got your money back."] remoteExec ["grad_lbm_fnc_reimburse",0,false];
};

//detract 1 from stock
_hashKey = format ["%1_%2_%3", _baseConfigName, _categoryConfigName, _itemConfigName];
[GRAD_LBM_ITEMSTOCKS, _hashKey, _stock - 1] call CBA_fnc_hashSet;
publicVariable "GRAD_LBM_ITEMSTOCKS";

//get code
_code = compile ([(missionConfigFile >> "CfgGradBuymenu" >> _baseConfigName >> _categoryConfigName >> _itemConfigName >> "code"), "text", ""] call CBA_fnc_getConfigEntry);

//do different stuff based on category type
_type = toUpper ([(missionConfigFile >> "CfgGradBuymenu" >> _baseConfigName >> _categoryConfigName >> _itemConfigName >> "kindOf"), "text", ""] call CBA_fnc_getConfigEntry);
if (_type == "") then {_type = toUpper ([(missionConfigFile >> "CfgGradBuymenu" >> _baseConfigName >> _categoryConfigName >> "kindOf"), "text", ""] call CBA_fnc_getConfigEntry)};
switch (_type) do {
    case ("WEAPONS"): {
        [_buyer,_account,_price,_code,_baseConfigName, _categoryConfigName, _itemConfigName, _vehiclespawn] remoteExec ["grad_lbm_fnc_buyWeapon",0,false];
        [[_buyer,_itemConfigName],_code] call grad_lbm_fnc_callCodeServer;
    };

    case ("ITEMS"): {
        [_buyer,_account,_price,_code,_baseConfigName, _categoryConfigName, _itemConfigName, _vehiclespawn] remoteExec ["grad_lbm_fnc_buyItem",0,false];
        [[_buyer,_itemConfigName],_code] call grad_lbm_fnc_callCodeServer;
    };

    case ("WEARABLES"): {
        [_buyer,_account,_price,_code,_baseConfigName, _categoryConfigName, _itemConfigName, _vehiclespawn] remoteExec ["grad_lbm_fnc_buyWearable",0,false];
        [[_buyer,_itemConfigName],_code] call grad_lbm_fnc_callCodeServer;
    };

    case ("VEHICLES"): {
        [_buyer,_account,_price,_code,_baseConfigName,_categoryConfigName,_itemConfigName,_vehiclespawn] call grad_lbm_fnc_buyVehicle;
    };

    case ("UNITS"): {
        [_buyer,_account,_price,_code,_baseConfigName,_categoryConfigName,_itemConfigName,_vehiclespawn] call grad_lbm_fnc_buyUnit;
    };
    
    case ("FORTIFICATION"): {
        if (!isNil "grad_fortifications_fnc_addFort") then {
            [_vehiclespawn, _itemConfigName] call grad_fortifications_fnc_addFort;
            [[_buyer,_itemConfigName],_code] remoteExec ["grad_lbm_fnc_callCodeClient",0,false];
            "Your purchase is in the destination's fortification cargo." remoteExec ["systemChat",_buyer,false];
        } else {
            [_buyer,_account,_price,"GRAD Fortifications not installed! You got your money back."] remoteExec ["grad_lbm_fnc_reimburse",0,false];
        };
    };

    case ("OTHER"): {
        [_buyer,_itemConfigName] call _code;
        [[_buyer,_itemConfigName],_code] remoteExec ["grad_lbm_fnc_callCodeClient",0,false];
    };

    default {
        [_buyer,_account,_price,"Unknown item type. You got your money back."] remoteExec ["grad_lbm_fnc_reimburse",0,false];
    };
};

//track
if (missionNamespace getVariable  ["grad_lbm_trackingTag",""] != "") then {
    [_baseConfigName,_categoryConfigName,_itemConfigName] call GRAD_lbm_tracking_fnc_trackPurchase;
};

// --- A3M DEEP STAT TRACKING: Purchases ---
if (isPlayer _buyer && !isNil "A3M_LiveProfiles") then {
    private _buyerUID = getPlayerUID _buyer;
    if (_buyerUID != "") then {
        private _profile = A3M_LiveProfiles getOrDefault [_buyerUID, createHashMap];
        if (count _profile > 0) then {
            
            // Try to resolve the beautiful display name instead of raw classname
            private _displayName = _itemConfigName;
            try {
                if (isClass (configFile >> "CfgWeapons" >> _itemConfigName)) then { _displayName = getText(configFile >> "CfgWeapons" >> _itemConfigName >> "displayName"); };
                if (isClass (configFile >> "CfgVehicles" >> _itemConfigName)) then { _displayName = getText(configFile >> "CfgVehicles" >> _itemConfigName >> "displayName"); };
                if (isClass (configFile >> "CfgMagazines" >> _itemConfigName)) then { _displayName = getText(configFile >> "CfgMagazines" >> _itemConfigName >> "displayName"); };
                if (isClass (configFile >> "CfgGlasses" >> _itemConfigName)) then { _displayName = getText(configFile >> "CfgGlasses" >> _itemConfigName >> "displayName"); };
            } catch {};
            
            private _lastPurchases = _profile getOrDefault ["Last_10_Purchases", []];
            private _purchaseEntry = [serverTime, _displayName, _price];
            _lastPurchases insert [0, [_purchaseEntry]];
            if (count _lastPurchases > 10) then { _lastPurchases resize 10; };
            _profile set ["Last_10_Purchases", _lastPurchases];
            
            A3M_LiveProfiles set [_buyerUID, _profile];
            ["A3M_PROFILE_" + _buyerUID, _profile] call A3M_fnc_dbSetSecure;
            diag_log format ["[A3M LEDGER] UID %1 bought %2 for %3", _buyerUID, _displayName, _price];
        };
    };
};
// -----------------------------------------
