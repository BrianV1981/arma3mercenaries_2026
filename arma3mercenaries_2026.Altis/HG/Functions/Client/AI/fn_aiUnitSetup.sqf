...
if((getNumber(getMissionConfig "CfgClient" >> "enableKillReward")) isEqualTo 1) then
{
    ...
    [
        "Killed",
        {
			...
			// A3M FIX: Route rewards to the human leader if AI got the kill
			private _human = if (isPlayer _instigator) then { _instigator } else { leader _instigator };
			if (!isPlayer _human) exitWith {};
            ...
            if((side (group _unit)) isEqualTo (side (group _instigator))) then
            {
                ...
                {
                    [(getNumber(getMissionConfig "CfgClient" >> "HG_MasterCfg" >> (rank _unit) >> "tkPenaltyAI")),1] remoteExecCall ["HG_fnc_addOrSubCash",_human,false];
					if((getNumber(getMissionConfig "CfgClient" >> "enableXP")) isEqualTo 1) then
					{
						[(getNumber(getMissionConfig "CfgClient" >> "HG_MasterCfg" >> (rank _unit) >> "xpPenaltyAI")),1] remoteExecCall ["HG_fnc_addOrSubXP",_human,false];
					};
                };
            } else {
			    if(395180 in (getDLCs 1)) then
				{
                    ...
                    [(getNumber(getMissionConfig "CfgClient" >> "HG_MasterCfg" >> (rank _unit) >> "killedReward")),0] remoteExecCall ["HG_fnc_addOrSubCash",_human,false];
				};
				if((getNumber(getMissionConfig "CfgClient" >> "enableXP")) isEqualTo 1) then
				{
					[(getNumber(getMissionConfig "CfgClient" >> "HG_MasterCfg" >> (rank _unit) >> "xpReward")),0] remoteExecCall ["HG_fnc_addOrSubXP",_human,false];
				};
				...
				{
					[0] remoteExecCall ["HG_fnc_addOrSubKills",_human,false];
				};
            };
        }
    ];
