...
// Initialize Global Audio Arrays
[] execVM "arma3mercenaries\speech_overhaul\fn_initSpeechArrays.sqf";

// --- A3M MERCENARY PERSISTENCE HOOK (Issue 24 Bugfix) ---
A3M_fnc_serverRegisterMercenary = {
    params ["_unit", "_mercID"];
    if (!isServer) exitWith {};
    
    // Safety check to prevent double registration
    if (_unit getVariable ["a3m_isRegistered", false]) exitWith {};
    _unit setVariable ["a3m_isRegistered", true, true];

    // (Name is now synchronously injected via createUnit init string in fn_serverDeployMercenary.sqf)

    _unit addMPEventHandler ["MPKilled", {
        params ["_unit", "_killer", "_instigator", "_useEffects"];
        if (!isServer) exitWith {};
        
        private _killedMercID = _unit getVariable ["arma3mercenaries_aiUnit", ""];
        private _groupID = _unit getVariable ["arma3mercenaries_groupID", ""]; 
        
        if (_killedMercID != "") then {
            private _killedProfile = [format["A3M_MERC_%1", _killedMercID], createHashMap, true] call A3M_fnc_dbGetSecure;
            if (count _killedProfile > 0) then {
                _killedProfile set ["Deaths", (_killedProfile getOrDefault ["Deaths", 0]) + 1];
                _killedProfile set ["IsDead", true];
                _killedProfile set ["DeathDate", systemTimeUTC];
                
                private _cause = "Killed in Action";
                if (isNull _instigator && isNull _killer) then { _cause = "Accident / Unknown"; };
                if (!isNull _instigator) then {
                    if (isPlayer _instigator) then {
                        private _instUID = getPlayerUID _instigator;
                        if (_instUID == _groupID || _groupID == format ["arma3mercenaries_groupID_%1", _instUID]) then {
                            _cause = "Executed by Commander";
                        } else {
                            _cause = format ["Killed by Player: %1", name _instigator];
                        };
                    } else {
                        if (side _instigator == side _unit) then { _cause = "Friendly Fire (AI)"; };
                    };
                };
                _killedProfile set ["CauseOfDeath", _cause];
                [format["A3M_MERC_%1", _killedMercID], _killedProfile] call A3M_fnc_dbSetSecure;
            };
        };
    }];
};

// Auto-Register Restored Persistent Mercenaries (grad-persistence doesn't natively attach MPKilled or names)
[{
    if (!isServer) exitWith {};
    {
        private _mercID = _x getVariable ["arma3mercenaries_aiUnit", ""];
        if (_mercID != "" && !(_x getVariable ["a3m_isRegistered", false])) then {
            [_x, _mercID] call A3M_fnc_serverRegisterMercenary;
        };
    } forEach allUnits;
...
// Player Profile & Global Server Functions
...
A3M_fnc_serverSatelliteSweep = compileFinal (preprocessFileLineNumbers "arma3mercenaries\tasks\fn_serverSatelliteSweep.sqf");
