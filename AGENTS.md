...
4. **THE READ-ONLY MAIN MANDATE (HARD RULE):** You DO NOT edit the codebase directly. You are strictly forbidden from modifying any project files, configurations, or source code in the main root directory. You must ONLY make modifications to the codebase inside your currently assigned, isolated workspace branch (e.g., `workspace/issue-<id>/`). Furthermore, you MUST verify the true mission root inside your workspace before editing files to ensure you aren't modifying the wrong directory.
